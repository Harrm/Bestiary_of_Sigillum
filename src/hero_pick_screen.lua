...
function HeroPickScreen:init(player1, player2)
	...
	self.textbox:setTextSize(5, 256)
	self.textbox:setRect(-50, -50, 50, 50)
	...
	self.textbox:setString(string.format("ALL PICK"))
end
...
function HeroPickScreen:proccessInput(worldX, worldY)
	...
	self.players.first.heroes[1] = hero
	self.textbox:setString(self.players.first.heroes[1].name)
